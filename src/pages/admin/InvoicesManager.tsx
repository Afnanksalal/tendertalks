  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="p-2.5 bg-slate-800 border border-white/10 rounded-lg text-slate-300 hover:text-white hover:bg-slate-700 transition-colors"
          >
            <RefreshCw size={16} />
          </button>
          <button
            onClick={exportCSV}
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-800 border border-white/10 rounded-lg text-slate-300 hover:text-white hover:bg-slate-700 transition-colors text-sm"
          >
            <Download size={14} />
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="bg-slate-900/50 border border-white/10 rounded-xl p-3 sm:p-4 mb-4 sm:mb-6"
      >
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filteredPayments.map((p, idx) => {
            const statusStyle = statusConfig[p.status] || {
              ...
              >
                ...
                <div className="sm:hidden">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${statusStyle.color.split(' ')[0]}`}
                      >
                    ...
                    <p
                      className={`font-mono font-bold text-sm flex-shrink-0 ${p.status === 'refunded' ? 'text-neon-purple line-through' : 'text-neon-green'}`}
                    >
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${statusStyle.color}`}
                      >
                        {p.status}
                      </span>
                    </div>
                ...
                <div className="hidden sm:flex sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${statusStyle.color.split(' ')[0]}`}
                    >
                  ...
                  <div className="flex items-center gap-4 lg:gap-6 flex-wrap">
                    ...
                    <p
                      className={`font-mono font-bold ${p.status === 'refunded' ? 'text-neon-purple line-through' : 'text-neon-green'}`}
                    >
                      ₹{parseFloat(p.amount).toLocaleString()}
                    </p>
                    <span

...
import { 
  ArrowLeft, Clock, Calendar, User, Share2, Tag, Loader2, FileText, Check, Copy, 
  ExternalLink, ChevronRight, ChevronUp, BookOpen
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#030014]">
      ...
      <div className="pt-24 sm:pt-28 md:pt-32 pb-20">
        <article className="max-w-4xl mx-auto px-4 relative z-10">
          ...
          {blog.bannerUrl && (
            ...
            >
              <img src={blog.bannerUrl} alt={blog.title} className="w-full h-full object-cover" loading="eager" />
            </motion.div>
          )}
              ...
              {blog.creator && (
                <div className="flex items-center gap-2">
                  {blog.creator.avatarUrl ? (
                    <img src={blog.creator.avatarUrl} alt={blog.creator.name || ''} className="w-9 h-9 rounded-full object-cover ring-2 ring-white/10" />
                  ) : (
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">{tableOfContents.length} sections</span>
                </div>
                <ChevronRight size={18} className={`text-slate-400 transition-transform duration-200 ${showToc ? 'rotate-90' : ''}`} />
              </button>
              <AnimatePresence>
                {showToc && (
                  <motion.nav
                ...
                components={{
                  img: ({ src, alt, ...props }) => (
                    <figure className="my-8">
                      <img src={src} alt={alt || ''} loading="lazy" className="rounded-xl shadow-xl max-w-full mx-auto" {...props} />
                      {alt && alt !== 'image' && <figcaption className="text-center text-sm text-slate-500 mt-3 italic">{alt}</figcaption>}
                    </figure>
                  ),
                  a: ({ href, children, ...props }) => {
                    ...
                    return (
                      ...
                      >
                        {children}
                        {isExternal && <ExternalLink size={12} className="inline flex-shrink-0 opacity-70" />}
                      </a>
                    );
                  },
                  table: ({ children, ...props }) => (
                    <div className="overflow-x-auto my-8 rounded-xl border border-white/10 shadow-lg -mx-4 sm:mx-0">
                      <table className="w-full min-w-[400px]" {...props}>{children}</table>
                    </div>
                  ),
                  thead: (props) => <thead className="bg-slate-800/80" {...props} />,
                  th: (props) => <th className="px-4 py-3 text-left text-sm font-semibold text-white border-b border-white/10" {...props} />,
                  td: (props) => <td className="px-4 py-3 text-sm text-slate-300 border-b border-white/5" {...props} />,
                  ...
                  pre: ({ children, ...props }) => {
                    ...
                    return (
                      <div className="relative group my-8 -mx-4 sm:mx-0">
                        ...
                        <pre className="bg-slate-800/80 border border-white/5 rounded-none sm:rounded-xl p-4 pt-8 overflow-x-auto text-sm" {...props}>
                          {children}
                        </pre>
                        <button
                        ...
                        >
                          {copiedCode === codeContent ? <Check size={14} className="text-neon-green" /> : <Copy size={14} />}
                        </button>
                      </div>
                    );
                  },
                  code: ({ inline, className, children, ...props }: any) => {
                    if (inline) {
                      return <code className="text-neon-cyan bg-slate-800/80 px-1.5 py-0.5 rounded text-sm font-mono" {...props}>{children}</code>;
                    }
                    return <code className={`block text-slate-300 text-sm font-mono leading-relaxed ${className || ''}`} {...props}>{children}</code>;
                  },
                  blockquote: ({ children, ...props }) => {
                    ...
                    if (isNote) { borderColor = 'border-blue-400/50'; bgColor = 'bg-blue-400/5'; }
                    else if (isTip) { borderColor = 'border-green-400/50'; bgColor = 'bg-green-400/5'; }
                    else if (isImportant) { borderColor = 'border-purple-400/50'; bgColor = 'bg-purple-400/5'; }
                    else if (isWarning) { borderColor = 'border-amber-400/50'; bgColor = 'bg-amber-400/5'; }
                    else if (isCaution) { borderColor = 'border-red-400/50'; bgColor = 'bg-red-400/5'; }
                    
                    return (
                      <blockquote className={`border-l-4 ${borderColor} ${bgColor} pl-4 pr-4 py-3 my-6 rounded-r-xl text-slate-400`} {...props}>
                        {children}
                      </blockquote>
                    );
                  },
                  input: ({ type, checked, ...props }: any) => {
                    if (type === 'checkbox') {
                      return (
                        <span className={`inline-flex items-center justify-center w-5 h-5 mr-2 rounded border flex-shrink-0 ${checked ? 'bg-neon-green/20 border-neon-green/50' : 'border-white/20 bg-slate-800/50'}`}>
                  ...
                  li: ({ children, ...props }: any) => {
                    const hasCheckbox = Array.isArray(children) && children.some((child: any) => child?.props?.type === 'checkbox');
                    return <li className={hasCheckbox ? 'list-none flex items-start gap-1' : ''} {...props}>{children}</li>;
                  },
                  h1: ({ children, id, ...props }) => (
                    <h1 id={id} className="group text-2xl sm:text-3xl font-display font-bold text-white mt-12 mb-6 scroll-mt-24 flex items-center gap-2" {...props}>
                      {children}
                      {id && <a href={`#${id}`} className="opacity-0 group-hover:opacity-50 text-neon-cyan transition-opacity">#</a>}
                    </h1>
                  ),
                  h2: ({ children, id, ...props }) => (
                    <h2 id={id} className="group text-xl sm:text-2xl font-display font-bold text-white mt-10 mb-5 scroll-mt-24 flex items-center gap-2" {...props}>
                      {children}
                      {id && <a href={`#${id}`} className="opacity-0 group-hover:opacity-50 text-neon-cyan transition-opacity">#</a>}
                    </h2>
                  ),
                  h3: ({ children, id, ...props }) => (
                    <h3 id={id} className="group text-lg sm:text-xl font-display font-bold text-white mt-8 mb-4 scroll-mt-24 flex items-center gap-2" {...props}>
                      {children}
                      {id && <a href={`#${id}`} className="opacity-0 group-hover:opacity-50 text-neon-cyan transition-opacity">#</a>}
                    </h3>
                  ),
                  h4: ({ children, id, ...props }) => (
                    <h4 id={id} className="group text-base sm:text-lg font-display font-bold text-white mt-6 mb-3 scroll-mt-24 flex items-center gap-2" {...props}>
                      {children}
                      {id && <a href={`#${id}`} className="opacity-0 group-hover:opacity-50 text-neon-cyan transition-opacity">#</a>}
                    </h4>
                  ),
                  ...
                  ul: (props) => <ul className="text-slate-300 my-5 pl-6 space-y-2 list-disc marker:text-neon-cyan" {...props} />,
                  ol: (props) => <ol className="text-slate-300 my-5 pl-6 space-y-2 list-decimal marker:text-neon-cyan" {...props} />,
            ...
            {blog.creator && (
              <div className="bg-slate-900/50 border border-white/10 rounded-xl p-6 mb-8">
                <div className="flex items-start gap-4">
                  {blog.creator.avatarUrl ? (
                    <img src={blog.creator.avatarUrl} alt={blog.creator.name || ''} className="w-16 h-16 rounded-full object-cover ring-2 ring-white/10" />
                  ) : (
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Written by</p>
                    <h4 className="text-lg font-bold text-white mb-1">{blog.creator.name || 'Author'}</h4>
                    <p className="text-sm text-slate-400">Thanks for reading! If you enjoyed this article, consider sharing it with others.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <Button
                  ...
                  className={shareText === 'Copied!' ? 'text-neon-green border-neon-green/30 bg-neon-green/10' : ''}
                >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 sm:hidden bg-slate-900/95 border-t border-white/10 px-4 py-2 safe-area-inset">
        <div className="flex items-center justify-between text-xs text-slate-400">
          ...
          <span>{blog.readTime || Math.ceil((blog.content?.split(/\s+/).length || 0) / 200)} min read</span>
        </div>
        <div className="h-1 bg-slate-800 rounded-full mt-1 overflow-hidden">
          <div className="h-full bg-gradient-to-r from-neon-cyan to-neon-purple transition-all duration-150" style={{ width: `${readingProgress}%` }} />
        </div>
      </div>
    </div>
  );

...
const SeekIndicator: React.FC<{ direction: 'left' | 'right'; seconds: number }> = ({
  direction,
  seconds,
}) => (
  <motion.div
...
export const PodcastDetailPage: React.FC = () => {
    ...
    (clientX: number, containerWidth: number) => {
      ...
      if (mediaRef.current) {
        mediaRef.current.currentTime = Math.max(
          0,
          Math.min(duration, mediaRef.current.currentTime + seekSeconds)
        );
        setCurrentTime(mediaRef.current.currentTime);
      }
  ...
  const canDownload =
    ...
    hasAccess &&
    (podcast.isDownloadable || subscription?.plan?.allowDownloads);
  ...
  const handleDownload = async () => {
    ...
    if (!canDownload) {
      toast.error('Upgrade your plan to download');
      return;
    }
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: podcast.description?.slice(0, 100) || '',
          url,
        });
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#030014] pt-24 sm:pt-28 md:pt-32 pb-20 safe-area-inset">
      <SEO
        title={podcast.title}
        description={
          podcast.description?.slice(0, 160) || `Listen to ${podcast.title} on TenderTalks`
        }
        image={podcast.thumbnailUrl || `/api/og-image?title=${encodeURIComponent(podcast.title)}`}
        url={`/podcast/${podcast.slug}`}
        type="article"
        publishedTime={
          podcast.publishedAt ? new Date(podcast.publishedAt).toISOString() : undefined
        }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          <div className="lg:col-span-2">
            ...
            >
              {hasAccess && podcast.mediaUrl && showPlayer && podcast.mediaType === 'video' ? (
                ...
                <>
                  {podcast.thumbnailUrl ? (
                    ...
                    <div
                      className={`w-full h-full flex items-center justify-center ${podcast.mediaType === 'video' ? 'bg-neon-purple/10' : 'bg-neon-cyan/10'}`}
                    >
              ...
              <AnimatePresence>
                {doubleTapSide && <SeekIndicator direction={doubleTapSide} seconds={seekAmount} />}
              </AnimatePresence>
              ...
              {!isFullscreen && (
                <div className="absolute top-3 left-3 z-20">
                  <span
                    className={`px-2.5 py-1 backdrop-blur text-xs font-bold rounded-lg flex items-center gap-1.5 ${
                      podcast.mediaType === 'video'
              ...
              <AnimatePresence>
                {(!showPlayer ||
                  ...
                  (showControls && showPlayer && podcast.mediaType === 'video')) && (
                  <motion.div
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    exit={{ opacity: 0, scale: 0.8 }}
              ...
              <AnimatePresence>
                {showPlayer && hasAccess && isFullscreen && showControls && (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-0.5 sm:gap-2">
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            changeSpeed(
                              PLAYBACK_SPEEDS[
                                (PLAYBACK_SPEEDS.indexOf(playbackSpeed) + 1) %
                    ...
                    {/* Mobile hint */}
                    <p className="text-white/40 text-[10px] text-center mt-3 sm:hidden">
                      Double tap sides to seek • Tap to hide controls
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <AnimatePresence>
                {isFullscreen && showControls && (
                  <motion.div
                    ...
                    onTouchEnd={(e) => e.stopPropagation()}
                  >
                    <h3 className="text-white font-medium text-sm sm:text-lg truncate pr-4">
                      {podcast.title}
                    </h3>
            ...
            {showPlayer && hasAccess && !isFullscreen && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="mb-6 p-3 sm:p-4 bg-slate-900/90 backdrop-blur-md border border-white/10 rounded-xl"
              >
                {/* Audio Now Playing */}
                      ...
                      <AnimatePresence>
                        {showVolumeSlider && (
                          ...
                          >
                            ...
                            <button
                              onClick={toggleMute}
                              className="mt-2 text-xs text-slate-400 hover:text-white w-full text-center"
                            >
                              {isMuted ? 'Unmute' : 'Mute'}
                            </button>
                          </motion.div>
                    ...
                    <div className="relative speed-menu">
                      ...
                      <AnimatePresence>
                        {showSpeedMenu && (
                          ...
                          >
                            <p className="text-[10px] text-slate-500 px-2 mb-1 uppercase tracking-wider">
                              Speed
                            </p>
                ...
                {podcast.mediaType === 'video' && (
                  <p className="text-slate-500 text-[10px] text-center mt-3 sm:hidden">
                    Double tap video sides to seek 10s
                  </p>
                )}
              </motion.div>
            )}
            ...
            {/* Title & Meta */}
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-display font-bold text-white mb-3">
              {podcast.title}
            </h1>
            ...
            {(podcast as any).tags?.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-6">
                {(podcast as any).tags.map((tag: any) => (
                  <span
                    key={tag.id}
                    className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-full text-xs text-slate-400"
                  >
                    #{tag.name}
                  </span>
                ))}
            ...
            <div className="lg:sticky lg:top-24 bg-slate-900/50 border border-white/10 rounded-xl p-5">
              {hasAccess ? (
                ...
                <div>
                  ...
                  <Button className="w-full mb-3" onClick={handlePurchase} isLoading={isPurchasing}>
                    Purchase Now
                  </Button>
                  <div className="text-center">

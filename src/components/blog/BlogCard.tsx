...
export const BlogCard: React.FC<BlogCardProps> = ({
  ...
  return (
    <div className="group relative bg-slate-900/40 backdrop-blur-sm border border-white/5 rounded-xl md:rounded-2xl overflow-hidden hover:border-neon-purple/40 transition-all duration-300 hover:shadow-[0_0_30px_rgba(168,85,247,0.1)] md:hover:-translate-y-1 card-hover h-full flex flex-col">
      {/* Thumbnail */}
      <Link to={`/blog/${blog.slug}`} className="block relative aspect-[16/10] overflow-hidden">
        ...
        {blog.bannerUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-neon-purple/10">
            <FileText className="w-12 h-12 text-neon-purple/40" />
          </div>
        )}
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 bg-black/40 backdrop-blur-[2px]">
          <div className="w-14 h-14 rounded-full bg-white/10 border border-white/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-neon-purple hover:border-neon-purple hover:scale-110 transition-all shadow-[0_0_20px_rgba(0,0,0,0.3)]">
            <ArrowRight size={24} />
          </div>
        </div>
      ...
      <div className="p-4 md:p-5 flex-1 flex flex-col">
        {/* Author */}
        {blog.creator && (
          <span className="text-xs font-medium text-neon-purple mb-2 block flex items-center gap-1">
            <User size={12} />
        ...
        <Link to={`/blog/${blog.slug}`}>
          <h3 className="text-base md:text-lg font-bold text-white mb-2 line-clamp-2 leading-tight group-hover:text-neon-purple transition-colors">
            {blog.title}
          </h3>
        </Link>
      ...
      <Link 
        to={`/blog/${blog.slug}`}
        className="md:hidden absolute bottom-4 right-4 w-10 h-10 rounded-full bg-neon-purple text-white flex items-center justify-center shadow-lg shadow-neon-purple/30 touch-feedback z-30"

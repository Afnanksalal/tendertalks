...
export const BlogCard: React.FC<BlogCardProps> = ({
  ...
  return (
    <div className="group relative bg-slate-900/40 backdrop-blur-sm border border-white/5 rounded-xl md:rounded-2xl overflow-hidden hover:border-neon-cyan/40 transition-all duration-500 ease-out hover:shadow-[0_0_30px_rgba(0,240,255,0.1)] md:hover:-translate-y-1 card-hover h-full flex flex-col">
      {/* Thumbnail */}
      <Link to={`/blog/${blog.slug}`} className="block relative aspect-[16/10] overflow-hidden">
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out bg-black/40 backdrop-blur-[2px]">
          <div className="w-14 h-14 rounded-full bg-white/10 border border-white/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-neon-cyan hover:border-neon-cyan hover:text-black transition-all duration-300 ease-out shadow-[0_0_20px_rgba(0,0,0,0.3)] scale-90 group-hover:scale-100">
            <ArrowRight size={24} />
          </div>
        </div>

...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const [pendingRefund] = await db.select().from(schema.refundRequests)
      .where(and(
    ...
    return new Response(JSON.stringify({
      ...
      daysRemaining: Math.max(0, Math.ceil((new Date(subscription.currentPeriodEnd).getTime() - now.getTime()) / (1000 * 60 * 60 * 24))),
    }), { status: 200, headers });

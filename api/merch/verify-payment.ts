...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // Idempotency check
    const [existing] = await db.select().from(schema.merchOrders)
      .where(eq(schema.merchOrders.razorpayOrderId, razorpay_order_id)).limit(1);
    
    if (existing?.status === 'paid') {
      return new Response(JSON.stringify({ success: true, order: existing, message: 'Already processed' }), { status: 200, headers });
    }
    ...
    const [order] = await db.update(schema.merchOrders)
    ...
    // Update stock
    const orderItems = await db.select().from(schema.merchOrderItems).where(eq(schema.merchOrderItems.orderId, order.id));
    for (const item of orderItems) {
      const [merch] = await db.select().from(schema.merchItems).where(eq(schema.merchItems.id, item.merchItemId));
      if (merch?.stockQuantity !== null) {
        const newStock = Math.max(0, merch.stockQuantity - item.quantity);
        await db.update(schema.merchItems)
          .set({ stockQuantity: newStock, inStock: newStock > 0, updatedAt: new Date() })

...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, planId, action = 'new' } = await req.json();
    ...
    if (!isValid) return new Response(JSON.stringify({ error: 'Invalid signature' }), { status: 400, headers });
    ...
    if (action === 'new' || action === 'upgrade') {
      ...
      const [currentSub] = await db.select().from(schema.subscriptions)
        .where(and(eq(schema.subscriptions.userId, userId), eq(schema.subscriptions.status, 'active'))).limit(1);

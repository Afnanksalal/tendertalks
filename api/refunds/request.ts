...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (!subscriptionId && !purchaseId) {
      return new Response(JSON.stringify({ error: 'Subscription ID or Purchase ID required' }), { status: 400, headers });
    }
    ...
    const existingRequest = await db.select().from(schema.refundRequests)
      .where(and(
        eq(schema.refundRequests.userId, userId),
        eq(schema.refundRequests.status, 'pending'),
        subscriptionId ? eq(schema.refundRequests.subscriptionId, subscriptionId) : eq(schema.refundRequests.purchaseId, purchaseId!)
      ))
      .limit(1);

    if (existingRequest.length > 0) {
      return new Response(JSON.stringify({ error: 'A refund request is already pending for this item' }), { status: 400, headers });
    }
    ...
    if (subscriptionId) {
      ...
      const [paymentEntry] = await db.select().from(schema.paymentHistory)
        .where(and(
      ...
      if (!purchase) {
        return new Response(JSON.stringify({ error: 'Purchase not found or not completed' }), { status: 404, headers });
      }

      // Check refund window
      const purchaseAge = Math.ceil((now.getTime() - new Date(purchase.createdAt).getTime()) / (1000 * 60 * 60 * 24));

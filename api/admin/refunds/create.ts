...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (!paymentId) return new Response(JSON.stringify({ error: 'Payment ID required' }), { status: 400, headers });

    const [payment] = await db.select().from(schema.paymentHistory)
      .where(eq(schema.paymentHistory.razorpayPaymentId, paymentId)).limit(1);

    if (!payment) return new Response(JSON.stringify({ error: 'Payment not found' }), { status: 404, headers });
    ...
    if (processImmediately) {
      try {
        ...
        await db.update(schema.refundRequests)
          .set({ status: 'processed', razorpayRefundId: razorpayRefund.id, processedAt: now, updatedAt: now })
        ...
        await db.update(schema.refundRequests)
          .set({ adminNotes: `Error: ${error instanceof Error ? error.message : 'Unknown'}`, updatedAt: now })
          .where(eq(schema.refundRequests.id, refundRequest.id));

...
export default async function handler(req: Request) {
  ...
  const [admin] = await db.select().from(schema.users)
    .where(and(eq(schema.users.id, adminId), eq(schema.users.role, 'admin'))).limit(1);
  ...
  if (req.method === 'GET') {
    try {
      ...
      const filtered = status && status !== 'all' ? results.filter(r => r.refund.status === status) : results;
  ...
  if (req.method === 'POST') {
    try {
      ...
      const [refundRequest] = await db.select().from(schema.refundRequests)
        .where(eq(schema.refundRequests.id, refundId)).limit(1);

      if (!refundRequest) return new Response(JSON.stringify({ error: 'Not found' }), { status: 404, headers });
      if (!['pending', 'approved'].includes(refundRequest.status)) {
        return new Response(JSON.stringify({ error: 'Already processed' }), { status: 400, headers });
      }
      ...
      if (action === 'approve') {
        ...
        if (refundRequest.subscriptionId) {
          const [sub] = await db.select().from(schema.subscriptions).where(eq(schema.subscriptions.id, refundRequest.subscriptionId)).limit(1);
          paymentId = sub?.razorpayPaymentId || null;
        } else if (refundRequest.purchaseId) {
          const [purchase] = await db.select().from(schema.purchases).where(eq(schema.purchases.id, refundRequest.purchaseId)).limit(1);
          paymentId = purchase?.razorpayPaymentId || null;
        }

        if (!paymentId) {
          return new Response(JSON.stringify({ error: 'Payment ID not found', manualRefundRequired: true }), { status: 400, headers });
        }

        const razorpayRefund = await processRazorpayRefund(paymentId, parseFloat(refundRequest.amount), { refundRequestId: refundId });

        await db.update(schema.refundRequests)
          .set({ status: 'processed', razorpayRefundId: razorpayRefund.id, processedBy: adminId, processedAt: now, adminNotes, updatedAt: now })
          .where(eq(schema.refundRequests.id, refundId));

        if (refundRequest.subscriptionId) {
          await db.update(schema.subscriptions).set({ status: 'cancelled', cancelledAt: now, updatedAt: now })
            .where(eq(schema.subscriptions.id, refundRequest.subscriptionId));
        }
        if (refundRequest.purchaseId) {
          await db.update(schema.purchases).set({ status: 'refunded', updatedAt: now })
            .where(eq(schema.purchases.id, refundRequest.purchaseId));
        }

        return new Response(JSON.stringify({ success: true, razorpayRefundId: razorpayRefund.id }), { status: 200, headers });

      } else if (action === 'mark_processed') {
        await db.update(schema.refundRequests)
          .set({ status: 'processed', razorpayRefundId: razorpayRefundId || null, processedBy: adminId, processedAt: now, adminNotes, updatedAt: now })
          .where(eq(schema.refundRequests.id, refundId));
        ...
        return new Response(JSON.stringify({ success: true, message: 'Marked as processed' }), { status: 200, headers });

      } else if (action === 'reject') {
        await db.update(schema.refundRequests)
          .set({ status: 'rejected', processedBy: adminId, processedAt: now, adminNotes, updatedAt: now })
          .where(eq(schema.refundRequests.id, refundId));
        return new Response(JSON.stringify({ success: true, message: 'Rejected' }), { status: 200, headers });
      }

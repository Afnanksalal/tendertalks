...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const [publishedPodcastCount] = await db
      .select({ count: count() })
    ...
    // Blog views
    const [totalBlogViews] = await db
      .select({ total: sum(schema.blogs.viewCount) })
      .from(schema.blogs);

    // Total views across all podcasts
    const [totalViews] = await db
      .select({ total: sum(schema.podcasts.viewCount) })
      .from(schema.podcasts);

    // Total plays from play history
    const [totalPlays] = await db.select({ count: count() }).from(schema.playHistory);
    const [completedPlays] = await db
      .select({ count: count() })
    ...
    const [purchaseRevenue] = await db
      .select({ total: sum(schema.purchases.amount), count: count() })
    ...
    const [merchRevenue] = await db
      .select({ total: sum(schema.merchOrders.totalAmount), count: count() })
      .from(schema.merchOrders)
      .where(eq(schema.merchOrders.status, 'paid'));

    // Last 30 days revenue (purchases + subscriptions + merch)
    const [recentPurchaseRevenue] = await db
      .select({ total: sum(schema.purchases.amount), count: count() })
      .from(schema.purchases)
      .where(
        and(
          eq(schema.purchases.status, 'completed'),
          gte(schema.purchases.createdAt, thirtyDaysAgo)
        )
      );

    const [recentSubRevenue] = await db
      .select({ total: sum(schema.subscriptions.amount), count: count() })
      .from(schema.subscriptions)
      .where(gte(schema.subscriptions.createdAt, thirtyDaysAgo));

    const [recentMerchRevenue] = await db
      .select({ total: sum(schema.merchOrders.totalAmount), count: count() })
      .from(schema.merchOrders)
      .where(
        and(eq(schema.merchOrders.status, 'paid'), gte(schema.merchOrders.createdAt, thirtyDaysAgo))
      );

    // Previous 30 days for comparison (30-60 days ago)
    const [prevPurchaseRevenue] = await db
    ...
    const [newUsersPrevMonth] = await db
      .select({ count: count() })
      .from(schema.users)
      .where(
        and(gte(schema.users.createdAt, sixtyDaysAgo), lte(schema.users.createdAt, thirtyDaysAgo))
      );
    ...
    const [pendingRefunds] = await db
      ...
      .where(eq(schema.refundRequests.status, 'pending'));

    // Total refunded amount
    const [refundedAmount] = await db
      .select({ total: sum(schema.refundRequests.amount) })
      .from(schema.refundRequests)
      .where(eq(schema.refundRequests.status, 'processed'));

    // Newsletter subscribers
    const [newsletterCount] = await db
      .select({ count: count() })
      .from(schema.newsletterSubscribers)
      .where(sql`${schema.newsletterSubscribers.unsubscribedAt} IS NULL`);

    // Average order value
    const avgPurchaseValue =
      purchaseRevenue.count > 0
    ...
    const topPodcasts = await db
      ...
      .groupBy(
        schema.podcasts.id,
        schema.podcasts.title,
        schema.podcasts.slug,
        schema.podcasts.thumbnailUrl,
        schema.podcasts.viewCount,
        schema.podcasts.isFree,
        schema.podcasts.price
      )
    ...
    const topMerch = await db
      ...
      .from(schema.merchItems)
      .leftJoin(
        schema.merchOrderItems,
        eq(schema.merchOrderItems.merchItemId, schema.merchItems.id)
      )
      .groupBy(
        schema.merchItems.id,
        schema.merchItems.name,
        schema.merchItems.imageUrl,
        schema.merchItems.price,
        schema.merchItems.stockQuantity
      )
    ...
    const userGrowth =
      Number(newUsersPrevMonth.count) > 0
        ? (
            ((Number(newUsersThisMonth.count) - Number(newUsersPrevMonth.count)) /
              Number(newUsersPrevMonth.count)) *
            100
          ).toFixed(1)
        : Number(newUsersThisMonth.count) > 0
          ? '100'
          : '0';

    // Conversion rate (users who made a purchase / total users)
    const [usersWithPurchases] = await db
      .select({ count: sql<number>`COUNT(DISTINCT ${schema.purchases.userId})` })
      .from(schema.purchases)
      .where(eq(schema.purchases.status, 'completed'));
